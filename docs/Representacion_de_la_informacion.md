...
2.  **Valores:** La materia prima (números, caracteres, señales) sobre la que operan dichas instrucciones.
...
Cuando un dato ocupa más de un byte (como el entero de 32 bits de la tabla anterior), surge una decisión de diseño crítica: **¿en qué orden guardamos los bytes en las celdas de memoria?** 

El procesador debe decidir qué byte va en la dirección de memoria más baja. Existen dos estándares:

1.  **Big-Endian (Orden "Humano"):** El byte **más significativo** (el que tiene más valor) se guarda en la dirección más baja. Es el orden natural en el que escribimos los números. Es el estándar en protocolos de Internet (llamado en este contexto Network Byte Order) y grandes sistemas bancarios (Mainframes).
2.  **Little-Endian (Orden de la Máquina):** El byte **menos significativo** se guarda en la dirección más baja. Es el estándar usado por los procesadores **Intel (x86)** y **ARM** (Apple Silicon, móviles). Aunque parezca contraintuitivo, facilita ciertas operaciones lógicas a nivel de circuitos.

!!! example "Comparativa: Guardar el número `0x12345678` en la dirección 2000"
...
!!! info "Nota importante: ¿Y los bits?"
    El *endianness* afecta únicamente al orden de los **bytes**. Los bits dentro de cada byte **no se giran**; mantienen siempre su orden posicional interno.


Mientras trabajamos dentro de un mismo sistema (un PC o un móvil), el *endianness* es transparente: el procesador guarda y lee usando su propia lógica y todo encaja. El problema surge cuando **dos máquinas diferentes deben comunicarse**.

Para que el mundo digital no sea una *Torre de Babel*, Internet utiliza Big-Endian. Si un PC con Intel (Little-Endian) quiere enviar un dato por la red, debe *girar* el orden de los bytes de cualquier dato numérico o magnitud que ocupe más de un byte (como un int de 32 bits o un float) antes de enviarlos para que el resto del mundo lo entienda correctamente.



!!! example "Un ejemplo de fallo en comunicación industrial"
    Imagina un sensor de presión en una fábrica que usa el protocolo **Modbus**. Este sensor mide la presión y la envía a un procesador supervisor como un entero de 2 bytes.
    
    *   Si el sensor mide `1` (en hexadecimal `0x0001`) y es Big-Endian, enviará los bytes: `00` y luego `01`.
    *   Si el procesador (Little-Endian) no los gira, pensará que ha recibido el número `0x0100` (que en decimal es **256**).
    
    **Resultado:** ¡El sistema de control creerá que la presión es 256 veces mayor de lo que realmente es y activará una alarma de emergencia por error! Por eso, en industria, es crítico configurar el orden de los bytes antes de empezar a medir.


!!! info "Curiosidad: ¿Una guerra por un huevo?"
    El término **Endianness** no nació en un laboratorio, sino en la literatura. Proviene de la novela satírica ***Los viajes de Gulliver*** (1726), de Jonathan Swift. 
    
    En el libro, los habitantes de Liliput están divididos en dos bandos enemigos que llevan décadas en guerra por una disputa absurda: ¿por qué extremo se debe romper un huevo pasado por agua, por el lado más grande (**Big-Endians**) o por el lado más pequeño (**Little-Endians**)?
    
    En 1980, el informático **Danny Cohen** utilizó estos nombres en un famoso artículo titulado *"Sobre las guerras santas y una petición de paz"*, para bromear sobre la *pelea* entre los fabricantes de procesadores (como Intel y Motorola) que no se ponían de acuerdo en qué byte debía ir primero. La analogía fue tan acertada que el término se convirtió en el estándar oficial de la informática.
...
!!! tip "La Analogía del Reloj"
    ...
    *   **Opción B (Suma del Complemento):** Mover la aguja 60-10=50 minutos hacia adelante $\to$ Posición `:50`.
...
!!! example "Ejemplo Práctico"
    ...
    1.  Calcula sus *dígitos* en base $2^{30}$ y obtiene tres bloques: `437976919`, `87719511` y `107`.
    2.  Guarda internamente una lista con la **magnitud**: `[437976919, 87719511, 107]`.
    3. Utiliza un campo extra para indicar la longitud de esa lista y el signo: almacenará **3** si es positivo o **-3** si es negativo. Este valor se almacena en **Complemento a 2**. El signo de este campo es el signo del número original y su valor absoluto determina cuantos bloques (dígitos) tiene la lista.
...
!!! tip "Comparación eficiente en la ALU"
    Gracias a este orden, la **ALU** (Unidad Aritmético-Lógica) puede comparar dos números reales simplemente recorriendo los bits de izquierda a derecha:
    1. Primero compara el bit de **signo** (positivo/negativo).
    2. Si los signos son iguales, compara el **exponente** (mayor orden de magnitud).
    3. Si los exponentes son iguales, compara la **mantisa** (precisión decimal).
    Este proceso es muy eficiente, ya que la comparación se realiza igual que con enteros: basta con buscar el primer bit distinto de izquierda a derecha para decidir cuál es mayor, sin necesidad de cálculos adicionales.
...
!!! example "La importancia de representar el exponente en Exceso en lugar de en C2"
    ...
    **Caso 1: Usando Exponente en Complemento a 2 (Estrategia fallida)**
...
* **Distribución en la recta real**
    ...
    !!! tip "Densidad constante, precisión variable"
...
!!! danger "*Ejemplo catastrófico:* **Fallo del misil Patriot (1991)**"
    ...
    * **La Distancia Fatal:** en ese breve lapso de desfase ($0.34 \text{ s}$), un misil Scud viajando a Match 5 ($1676 \text{ m/s}$) recorre una distancia enorme:
...
!!! tip "Características de diseño ingeniosas"
    Los diseñadores de ASCII ordenaron los caracteres estratégicamente para facilitar el procesamiento por hardware o simplificar los algoritmos que trabajan con caracteres:
    ...
    2.  **Mayúsculas y Minúsculas:** 
        ...
        *   `'a'` = 97 (`0110 0001`)
        ...
        La única diferencia es el **bit 5**. Para pasar de mayúscula a minúscula basta con *activar* ese bit . Para pasar a mayúscula, basta con *desactivarlo*.
...
!!! example "De Code Point a Bytes"
    ...
    2.  **La letra 'ñ' `U+00F1`** ($241_{10}$):
        ...
        *   *Curiosidad:* si abres un archivo UTF-8 como si fuera Latin-1, la ñ se ve como dos caracteres extraños ("Ã±"): el ordenador interpreta `0xC3` ("Ã") y `0xB1` ("±") por separado.
    ...
    4.  **El Emoji '😂' `U+1F602`** ($128514_{10}$):
        ...
        *   **Paso a paso:**
            ...
            2.  Rellenamos con ceros hasta 21 bits: **`0`**`0001111101100000010`.
...
!!! success "Por qué ganó UTF-8"
    *   **Ahorro de espacio:**
        *   **Inglés:** Los textos clásicos (novelas, ensayos) y normalmente el código fuente (HTML, Python, C++) contienen solo caracteres ASCII. En UTF-8 ocupan exactamente **1 byte por carácter**, igual que en los años 70.
        *   **Español:** La sobrecarga es mínima. En un texto normal (como *El Quijote*), solo las vocales acentuadas y la 'ñ' ocupan 2 bytes. El 98% del texto (letras normales, espacios, puntuación) sigue ocupando 1 byte. El aumento total de tamaño suele rondar apenas el **2%**.
        ...
        Si usáramos UTF-16, el archivo ocuparía **el doble** de tamaño, ya que *cada letra* (incluida la 'a' o el espacio) gastaría 2 bytes obligatoriamente.
...
*   **Python 3:** El tipo `str` es **Unicode** por defecto: cada elemento de la cadena representa un símbolo Unicode (un code point), pero **la codificación interna en memoria no es UTF-8, UTF-16 ni UTF-32 estándar**. Desde Python 3.3, el lenguaje asocia cada símbolo Unicode a una codificación propia optimizada para ahorrar memoria, eligiendo 1, 2 o 4 bytes por carácter según el contenido de la cadena.
    ...
    *   Si hay algún carácter *raro* (emojis, símbolos históricos, etc.), usa 4 bytes por carácter (UTF-32).
    ...
    !!! example "Representar caracteres por punto de código en Python"
        En Python puedes crear un carácter a partir de su punto de código Unicode usando la función `chr()` o mediante literales con prefijo `\u` o `\U`:
        ```python
        # Usando chr() con el punto de código decimal o hexadecimal
        print(chr(241))        # 'ñ' (U+00F1)
        print(chr(0x1F602))   # '😂' (U+1F602)

        # Usando literales Unicode
        print('\u00F1')       # 'ñ'
        print('\U0001F602')   # '😂'

        # Obtener el punto de código de un carácter
        print(ord('ñ'))       # 241
        print(hex(ord('😂')))  # 0x1f602
        ```
        Así puedes trabajar directamente con cualquier símbolo Unicode en tus programas.
